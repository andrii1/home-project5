/* eslint-disable guard-for-in */
/* eslint-disable no-restricted-syntax */
/* TODO: This is an example controller to illustrate a server side controller.
Can be deleted as soon as the first real controller is added. */

const knex = require('../../config/db');
const HttpError = require('../lib/utils/http-error');
const generateSlug = require('../lib/utils/generateSlug');
const capitalize = require('../lib/utils/capitalize');
const getOppositeOrderDirection = require('../lib/utils/getOppositeOrderDirection');
// eslint-disable-next-line no-unused-vars
const OpenAI = require('openai');

const openai = new OpenAI({
  apiKey: process.env.OPENAI_API_KEY, // make sure this is set in your .env
});

// Helper: ensure the slug is unique by checking the DB
async function ensureUniqueSlug(baseSlug) {
  let slug = baseSlug;
  let counter = 1;

  // eslint-disable-next-line no-await-in-loop
  while (await slugExists(slug)) {
    const suffix = `-${counter}`;
    const maxBaseLength = 200 - suffix.length;
    slug = `${baseSlug.slice(0, maxBaseLength)}${suffix}`;
    counter += 1;
  }

  return slug;
}

// Helper: check if a slug already exists in the database
async function slugExists(slug) {
  const existing = await knex('chapters').where({ slug }).first();
  return !!existing;
}

// Helper: ensure the slug is unique by checking the DB
async function ensureUniqueSlugItems(baseSlug, table) {
  let slug = baseSlug;
  let counter = 1;

  // eslint-disable-next-line no-await-in-loop
  while (await slugExistsItems(slug, table)) {
    const suffix = `-${counter}`;
    const maxBaseLength = 200 - suffix.length;
    slug = `${baseSlug.slice(0, maxBaseLength)}${suffix}`;
    counter += 1;
  }

  return slug;
}

// Helper: check if a slug already exists in the database
async function slugExistsItems(slug, table) {
  const existing = await knex(table).where({ slug }).first();
  return !!existing;
}

async function createItems(prompt, table) {
  const completion = await openai.chat.completions.create({
    model: 'gpt-4o-mini',
    messages: [{ role: 'user', content: prompt }],
    temperature: 0.7,
    max_tokens: 3000,
  });

  const string = completion.choices[0].message.content.trim();

  const array = string
    .split(',')
    .map((tag) => (typeof tag === 'string' ? tag.trim() : null))
    .filter(Boolean);

  const itemsIds = await Promise.all(
    array.map(async (item) => {
      const slug = generateSlug(item);
      const existing = await knex(table).where({ slug }).first();

      if (existing) {
        return existing.id;
      }

      const [itemId] = await knex(table).insert({
        title: item,
        slug,
      }); // just use the ID
      return itemId;
    }),
  );
  return itemsIds;
}

const getChapters = async () => {
  try {
    const chapters = await knex('chapters')
      .select(
        'chapters.*',
        'users.email as userEmail',
        'users.full_name as userFullName',
      )
      .join('users', 'chapters.user_id', '=', 'users.id');

    return chapters;
  } catch (chapter) {
    return chapter.message;
  }
};

const pricingFiltersMap = {
  free: (qb) => qb.orWhere('chapters.pricing_free', true),
  freemium: (qb) => qb.orWhere('chapters.pricing_freemium', true),
  'ios-paid': (qb) => qb.orWhere('chapters.pricing_ios_chapter_paid', true),
  'ios-free': (qb) => qb.orWhere('chapters.pricing_ios_chapter_free', true),
  subscription: (qb) => qb.orWhere('chapters.pricing_subscription', true),
  'one-time': (qb) => qb.orWhere('chapters.pricing_one_time', true),
  trial: (qb) => qb.orWhere('chapters.pricing_trial_available', true),
};

const platformsFiltersMap = {
  'browser-extension': (qb) =>
    qb.orWhereNotNull('chapters.url_chrome_extension'),
  ios: (qb) => qb.orWhereNotNull('chapters.chapterle_id'),
  android: (qb) => qb.orWhereNotNull('chapters.url_google_play_store'),
  windows: (qb) => qb.orWhereNotNull('chapters.url_windows'),
  mac: (qb) => qb.orWhereNotNull('chapters.url_mac'),
};

const socialMediaFiltersMap = {
  twitter: (qb) => qb.orWhereNotNull('chapters.url_x'),
  discord: (qb) => qb.orWhereNotNull('chapters.url_discord'),
};

const otherFiltersMap = {
  'open-source': (qb) => qb.orWhere('chapters.is_open_source', true),
  ai: (qb) => qb.orWhere('chapters.is_ai_powered', true),
};

const getChaptersBy = async (params) => {
  const {
    page = 0,
    column = 'id',
    direction = 'asc',
    categories,
    socials,
    other,
    search,
    tags,
    keywords,
    highlights,
    userTypes,
    products,
  } = params;

  const lastItemDirection = direction === 'asc' ? 'desc' : 'asc';

  // --- Helper functions ---
  const applyMappedFilter = (qb, valueCSV, map) => {
    if (!valueCSV) return;
    const arr = valueCSV.split(',');
    qb.where(function () {
      arr.forEach((item) => {
        const fn = map[item];
        if (fn) fn(this);
      });
    });
  };

  const applyManyToManyFilter = (qb, valueCSV, joinTable, targetTable, key) => {
    if (!valueCSV) return;

    const arr = valueCSV.split(',');
    const foreignKey = foreignKeyMap[key];

    qb.whereIn('chapters.id', function () {
      this.select(`${joinTable}.chapter_id`)
        .from(joinTable)
        .join(targetTable, `${joinTable}.${foreignKey}`, `${targetTable}.id`)
        .whereIn(`${targetTable}.slug`, arr);
    });
  };

  const tableMap = {
    tags: 'tags',
    keywords: 'keywords',
    highlights: 'highlights',
    userTypes: 'userTypes',
  };

  const foreignKeyMap = {
    tags: 'tag_id',
    keywords: 'keyword_id',
    highlights: 'highlight_id',
    userTypes: 'userType_id',
  };

  const joinMap = {
    tags: 'tagsChapters',
    keywords: 'keywordsChapters',
    highlights: 'highlightsChapters',
    userTypes: 'userTypesChapters',
  };

  try {
    // --- Base query ---
    const getModel = () => {
      return knex('chapters')
        .select(
          'chapters.*',
          'categories.title as categoryTitle',
          'categories.slug as categorySlug',
          'products.title as productTitle',
          'products.slug as productSlug',
          knex.raw(`(
        SELECT COUNT(*)
        FROM favorites
        WHERE favorites.chapter_id = chapters.id
      ) as favoritesCount`),

          knex.raw(`(
        SELECT COUNT(*)
        FROM ratings
        WHERE ratings.chapter_id = chapters.id
      ) as ratingsCount`),
        )
        .modify((qb) => {
          // --- Simple filters ---
          if (categories) qb.whereIn('categories.slug', categories.split(','));
          if (products) qb.whereIn('products.slug', products.split(','));
          applyMappedFilter(qb, socials, socialMediaFiltersMap);
          applyMappedFilter(qb, other, otherFiltersMap);

          // --- Search ---
          if (search) {
            const arr = search.split(',');
            qb.where(function () {
              arr.forEach((term) => {
                this.orWhere('chapters.title', 'like', `%${term}%`)
                  .orWhere('chapters.content', 'like', `%${term}%`)
                  .orWhere('chapters.summary', 'like', `%${term}%`);
              });
            });
          }

          // --- Many-to-many filters ---
          const manyToMany = {
            tags,
            keywords,
            highlights,
            userTypes,
          };
          for (const key in manyToMany) {
            applyManyToManyFilter(
              qb,
              manyToMany[key],
              joinMap[key],
              tableMap[key],
              key,
            );
          }
        });
    };

    // --- Sorting ---
    const applySorting = (qb, col, dir) => {
      if (col === 'highestRated') qb.orderBy('ratingsCount', dir);
      else if (col === 'mostBookmarked') qb.orderBy('favoritesCount', dir);
      else qb.orderBy(`chapters.${col}`, dir);
    };

    const baseQuery = getModel();

    // --- Data query ---
    const dataQuery = baseQuery
      .clone()
      .modify((qb) => applySorting(qb, column, direction))
      .offset(page * 10)
      .limit(10);

    // --- Last item for pagination ---
    const lastQuery = baseQuery
      .clone()
      .modify((qb) => applySorting(qb, column, lastItemDirection))
      .limit(1);

    const data = await dataQuery;
    const lastItem = await lastQuery;

    return { lastItem: lastItem[0], data };
  } catch (error) {
    return error.message;
  }
};

const getChaptersPagination = async (page, column, direction) => {
  const lastItemDirection = getOppositeOrderDirection(direction);
  try {
    const getModel = () => knex('chapters');
    const lastItem = await getModel()
      .orderBy(column, lastItemDirection)
      .limit(1);
    const data = await getModel()
      .orderBy(column, direction)
      .offset(page * 10)
      .limit(10)
      .select();
    return {
      lastItem: lastItem[0],
      data,
    };
  } catch (chapter) {
    return chapter.message;
  }
};

const getChapterById = async (slug) => {
  if (!slug) {
    throw new HttpError('Id should be a number', 400);
  }

  try {
    const chapter = await knex('chapters')
      .select('chapters.*')
      .where('chapters.slug', slug);
    if (chapter.length === 0) {
      throw new Error(`incorrect entry with the id of ${slug}`, 404);
    }
    return chapter;
  } catch (error) {
    return error.message;
  }
};

// const editChapter = async (exampleResourceId, updatedExampleResource) => {
//   if (!exampleResourceId) {
//     throw new HttpChapter('exampleResourceId should be a number', 400);
//   }

//   return knex('chapters').where({ id: exampleResourceId }).update({
//     title: updatedExampleResource.title,
//     updatedAt: moment().format(),
//   });
// };

// const deleteExampleResource = async (exampleResourceId) => {
//   return knex('exampleResources').where({ id: exampleResourceId }).del();
// };

const createChapterNode = async (token, body) => {
  try {
    const userUid = token.split(' ')[1];
    const user = (await knex('users').where({ uid: userUid }))[0];
    if (!user) throw new HttpError('User not found', 401);

    // === Check for existing chapters ===

    const slugChapter = generateSlug(body.title);
    const existingChapter = await knex('chapters')
      .where({ slug: slugChapter })
      .first();

    if (existingChapter)
      return {
        successful: true,
        existing: true,
        chapterId: existingChapter.id,
        chapterTitle: body.title,
      };

    // === Tags ===
    const promptTags = `Create 3-5 niche, detailed, long tail tags for this chapter: "${body.title}. Tag should be without hashtag, multiple words allowed, and not contain 'chapter'. Return tags separated by comma.`;
    const tagsString = (
      await openai.chat.completions.create({
        model: 'gpt-4o-mini',
        messages: [{ role: 'user', content: promptTags }],
        temperature: 0.7,
        max_tokens: 3000,
      })
    ).choices[0].message.content.trim();

    const tagsArray = tagsString
      .split(',')
      .map((t) => t.trim())
      .filter(Boolean);
    if (body.tag) tagsArray.push(body.tag);

    const tagIds = await Promise.all(
      tagsArray.map(async (tag) => {
        const slug = await generateSlug(tag);
        const existingTag = await knex('tags').where({ slug }).first();

        if (existingTag) return existingTag.id;

        const [tagId] = await knex('tags').insert({
          title: tag,
          slug,
        });
        return tagId;
      }),
    );

    // === Prepare chapter info ===

    const completion = await openai.chat.completions.create({
      model: 'gpt-4o-mini',
      messages: [
        {
          role: 'user',
          content: `Create a content-solution for this chapter: "${body.title}". Try to answer questions like "How to solve ${body.title}?". Browse recent threads in reddit, forums, support forums, social media threads about chapter "${body.title}". Summarize user experience and what are possible solutions for chapter "${body.title}". Treat ${body.title} as main keyword - it should be spread in the blog. At least 1300 words. Do not include published by [Your Name] or Published on [Date]. Do not include title, headline, h1, h2 of the blog, just content of the article. Output with markdown.`,
        },
      ],
      temperature: 0.7,
      max_tokens: 3000,
    });
    const content = completion.choices[0].message.content.trim();

    const completionMetaDescription = await openai.chat.completions.create({
      model: 'gpt-4o-mini',
      messages: [
        {
          role: 'user',
          content: `Write a short, engaging meta description SEO for chapter "${body.title}. Do not include link in description. Maximum 150 characters.`,
        },
      ],
      temperature: 0.7,
      max_tokens: 3000,
    });
    const metaDescription =
      completionMetaDescription.choices[0].message.content.trim();

    // Generate a short description using OpenAI
    const promptSummary = `Write a short, 200 characters maximum, chapter summary for chapter with title "${body.title}" and content "${body.content}". Do not include "**Chapter Summary:**" part.`;

    const completionSummary = await openai.chat.completions.create({
      model: 'gpt-4o-mini',
      messages: [{ role: 'user', content: promptSummary }],
      temperature: 0.7,
      max_tokens: 600,
    });

    const description = completionSummary.choices[0].message.content.trim();

    //     // === Pricing + Attributes ===

    //     const [pricingData, attributesData, faqData] = await Promise.all([
    //       useChatGptForData(
    //         `Given the chapter "${body.title}"${
    //           chapterUrl ? ` with website ${chapterUrl}` : ''
    //         }${
    //           description ? ` and description: \"${description}\"` : ''
    //         }, determine its pricing model.

    // Return JSON with keys:
    // {
    //   "pricing_freemium": true/false,
    //   "pricing_subscription": true/false,
    //   "pricing_one_time": true/false,
    //   "pricing_trial_available": true/false,
    //   "pricing_details": "short human-readable text about pricing, e.g. '$9/mo or $89/year'",
    //   "pricing_url": "official pricing page URL if available, otherwise null",
    //   "pricing_free": true/false
    // }

    // Respond ONLY with valid JSON.`,
    //       ),

    //       useChatGptForData(
    //         `Based on the chapter "${body.title}"${
    //           chapterUrl ? ` with website ${chapterUrl}` : ''
    //         }${
    //           description ? ` and description: \"${description}\"` : ''
    //         }, determine if:

    // Return JSON with keys:
    // {
    //   "is_ai_powered": true/false,
    //   "is_open_source": true/false,
    //   "url_chrome_extension": url for browser extension (if available),
    //   "url_google_play_store": url for android chapter (if available),
    //   "url_windows": url for windows chapter (if available),
    //   "url_mac": url for mac chapter (if available),
    //   "url_x": url for X/twitter account (if available),
    //   "url_discord": url for discord account (if available),
    //   "url_fb": url for Facebook account (if available),
    //   "url_linkedin": url for linkedin account (if available),
    //   "e-mail": e-mail (if available, can be support e-mail),
    // }

    // Respond ONLY with valid JSON.`,
    //       ),

    //       useChatGptForData(
    //         `Based on the chapter "${body.title}"${
    //           chapterUrl ? ` with website ${chapterUrl}` : ''
    //         }${
    //           description ? ` and description: \"${description}\"` : ''
    //         }, determine if:

    // - How to create an account in chapter "${body.title}".
    // - How to delete an account in chapter "${body.title}".
    // - How to contact support in chapter "${body.title}".
    // - How to cancel subscription for chapter "${body.title}".
    // - How to change profile picture in chapter "${body.title}".
    // - How to log in "${body.title}".
    // - How to log out "${body.title}".
    // - Is chapter "${body.title}" on Android?
    // - Chapter "${body.title}" doesn't work? Any common bugs? How to solve them?
    // - Is chapter "${body.title}" safe to use? Is it legit or scammy?
    // - Can you make money with chapter "${body.title}"?
    // - Does it make sense to upgrade in chapter "${
    //           body.title
    //         }"? What are main highlights of premium version.
    // - Can you use chapter "${
    //           body.title
    //         }" for free? Any ways to credits/coins for free? Either via promos, invite codes, completing tasks, etc.
    // - How to use "${body.title}"? Longer description.

    // Return JSON with keys:
    // {
    //     "faq_create_account": answer,
    //     "faq_delete_account": answer,
    //     "faq_contact_support": answer,
    //     "faq_cancel_subscription": answer,
    //     "faq_change_profile_picture": answer,
    //     "faq_log_in": answer,
    //     "faq_log_out": answer,
    //     "faq_is_chapter_on_android": answer,
    //     "faq_chapter_doesnt_work_bugs": answer,
    //     "faq_is_safe_to_use": answer,
    //     "faq_how_to_make_money": answer,
    //     "faq_should_you_upgrade": answer,
    //     "faq_can_use_for_free": answer,
    //     "description_how_to_use": answer,

    // }

    // Respond ONLY with valid JSON.`,
    //       ),
    //     ]);

    // === Insert chapter ===

    const [chapterId] = await knex('chapters').insert({
      ...body,
      slug: slugChapter,
      meta_description: metaDescription,
      content,
      summary: description,
    });

    // const [chapterId] = await knex('chapters').insert({
    //   external_id: body.external_id,
    //   title: body.title,
    //   slug: uniqueSlug,
    //   price: body.price,
    //   rating: body.rating,
    //   reviews: body.reviews,
    //   url: body.url,
    //   url_affiliate: body.url_affiliate,
    //   discount_percentage: body.discount_percentage,
    //   category_id: body.category_id,
    //   city_id: body.city_id,
    //   platform_id: body.platform_id,
    //   description_ai: description,
    //   meta_description: metaDescription,
    // });

    // === Prompt builder ===
    const buildPrompt = (type, title, url, descriptionParam, quantity) => {
      const examples = {
        highlights:
          'E.g. Network connection issue, Restart device, Clear app cache',
        userTypes: 'E.g. Individuals, Teams, Students',
      };

      let base = `for this chapter: \"${title}\"`;
      if (url) base += ` with website ${url}`;
      if (descriptionParam) base += ` and description: \"${descriptionParam}\"`;

      return `Create niche, detailed, long tail ${type} ${base}. ${
        examples[type]
      }. ${capitalize(
        type,
      )} should be without hashtag, can be multiple words. Maximum ${quantity} ${type}. Return ${type} separated by comma.`;
    };

    // === Highlights, UserTypes, Occasions, UseCases, Industries ===
    const highlightsIds = await createItems(
      buildPrompt('highlights', body.title, body.url, description, '5'),
      'highlights',
    );
    const userTypesIds = await createItems(
      buildPrompt('userTypes', body.title, body.url, description, '5'),
      'userTypes',
    );

    // === Relations ===
    const insertRelations = async (table, key, ids) =>
      Promise.all(
        ids.map((id) =>
          knex(table).insert({ chapter_id: chapterId, [key]: id }),
        ),
      );
    await insertRelations('tagsChapters', 'tag_id', tagIds);
    await insertRelations('highlightsChapters', 'highlight_id', highlightsIds);
    await insertRelations('userTypesChapters', 'userType_id', userTypesIds);

    return {
      successful: true,
      chapterId,
      external_id: body.external_id,
      chapterTitle: body.title,
      url: body.url,
    };
  } catch (error) {
    return error.message;
  }
};

// const createChapter = async (token, body) => {
//   try {
//     const userUid = token.split(' ')[1];
//     const user = (await knex('users').where({ uid: userUid }))[0];
//     if (!user) {
//       throw new HttpChapter('User not found', 401);
//     }

//     const baseSlug = generateSlug(body.title);
//     const uniqueSlug = await ensureUniqueSlug(baseSlug);

//     // Generate a short description using OpenAI
//     const prompt = `Write a short, 200 characters maximum, chapter summary for chapter with title "${body.title}" and content "${body.content}". Do not include "**Chapter Summary:**" part.`;

//     const completion = await openai.chat.completions.create({
//       model: 'gpt-4o-mini',
//       messages: [{ role: 'user', content: prompt }],
//       temperature: 0.7,
//       max_tokens: 600,
//     });

//     const summary = completion.choices[0].message.content.trim();

//     const promptTags = `Create 3-4 long-tail, niche tags for this chapter with title: "${body.title}" and content "${body.content}". Tag should be without hashtag, ideally one word, which describes the chapter, but can be from more words if needed in context. Return tags separated by comma.`;

//     const completionTags = await openai.chat.completions.create({
//       model: 'gpt-4o-mini',
//       messages: [{ role: 'user', content: promptTags }],
//       temperature: 0.7,
//       max_tokens: 600,
//     });

//     const tagsString = completionTags.choices[0].message.content.trim();

//     const tagsArray = tagsString.split(',').map((tag) => tag.trim());

//     if (body.tag) {
//       tagsArray.push(body.tag);
//     }

//     const tagIds = await Promise.all(
//       tagsArray.map(async (tag) => {
//         const existingTag = await knex('tags')
//           .whereRaw('LOWER(title) = ?', [tag.toLowerCase()])
//           .first();

//         if (existingTag) {
//           return existingTag.id;
//         }
//         const baseSlugTag = generateSlug(tag);
//         const uniqueSlugTag = await ensureUniqueSlug(baseSlugTag);
//         const [tagId] = await knex('tags').insert({
//           title: tag,
//           slug: uniqueSlugTag,
//         }); // just use the ID
//         return tagId;
//       }),
//     );

//     const [chapterId] = await knex('chapters').insert({
//       title: body.title,
//       content: body.content,
//       slug: uniqueSlug,
//       summary,
//       cover_image_url: body.cover_image_url,
//       status: body.status,
//       created_at: body.created_at,
//       updated_at: body.updated_at,
//       meta_description: body.meta_description,
//       user_id: body.user_id,
//     });

//     const insertedChapterToTags = await Promise.all(
//       tagIds.map((tagId) =>
//         knex('tagsChapters').insert({
//           chapter_id: chapterId,
//           tag_id: tagId,
//         }),
//       ),
//     );

//     return {
//       successful: true,
//       chapterId,
//     };
//   } catch (chapter) {
//     return chapter.message;
//   }
// };

module.exports = {
  getChapters,
  getChaptersPagination,
  getChapterById,
  getChaptersBy,
  // deleteExampleResource,
  // createChapter,
  createChapterNode,
  // editExampleResource,
};
