...
const getFavoritesByUserId = async (token) => {
  ...
  try {
    const favorites = await knex('chapters')
      .select('chapters.*', 'favorites.id as favoritesID')
      .leftJoin('favorites', function () {
        this.on('chapters.id', '=', 'favorites.chapter_id');
      })
...
const deleteFavorites = async (token, favoritesId) => {
  ...
  try {
    const deletedFav = await knex('favorites')
      .where({ chapter_id: favoritesId, user_id: user.id })
      .del();
    if (deletedFav === 0) {
